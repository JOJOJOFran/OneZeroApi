...
namespace SouthStar.VehSch.Api.Areas.Setting.Controllers
{
    ...
    public class VehicleController:BaseController
    {
        ...
        /// <returns></returns>      
        [HttpPost]
        public async Task<IActionResult> Add([FromBody]VehicleData value)
        {
            if (value == null)
            {
                return Json(BadParameter("车辆信息内容不能为空"));
            }
            var dto = await _vehicleManageService.AddAsync(value);
            return Json(dto);
        }
        ...
        /// <returns></returns>
        [HttpPost("{vehicleId}")]
        public async Task<IActionResult> Update(string vehicleId, [FromBody]VehicleData value)
        {
            if (!Guid.TryParse(vehicleId, out _id))
            {
                return Json(BadParameter("Id格式不匹配"));
            ...
            var dto = await _vehicleManageService.UpdateAsync(_id, value);
            return Json(dto);
        }
