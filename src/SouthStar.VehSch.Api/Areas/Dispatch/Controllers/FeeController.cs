...
namespace SouthStar.VehSch.Api.Areas.Dispatch.Controllers
{
    public class FeeController : BaseController
    {
        ...
        /// <param name="DispatchFeeService"></param>
        ...
        {
            var vehicleList = await _feeService.GetFeeListAsync(applyNum, startDate, endDate, limit);
        ...
        public async Task<IActionResult> Add([FromBody]object value)
        {
            ...
            var fee = JsonConvert.DeserializeObject<DispatchFeeData>(value.ToString());
            var dto = await _feeService.AddFeeAsync(fee);
            return Json(dto);
        }
        ...
        /// <returns></returns>
        [HttpPost("{feeId}")]
        public async Task<IActionResult> Update(string feeId, [FromBody]object value)
        {
            ...
            var fee = JsonConvert.DeserializeObject<DispatchFeeData>(value.ToString());
            var dto = await _feeService.UpdateFeeAsync(_id, fee);
            return Json(dto);
        }
    }
}
